...
{
  // Ignore small words
  const std::size_t small_word_bound{ 3 };        // Set to 0 to not ignore small words at all
  ...
  for (std::size_t i {}; i < text.length(); )
  {
  ...
  std::size_t counts[26] {}; // All counts are initialized to 0
  std::size_t long_word_count{};
  ...
  std::size_t most_common_index{ 0 };
  for (std::size_t i{ 1 }; i < std::size(counts); ++i)
  {
  ...
  {
    std::println("The text that you entered is no tautogram. Not even close.");
  }
}
